 ...
 * of the ELF file.
 * @argc: The number of arguments suppliedn to the program
 * @argv: An array of pointers to the arguments.
 *
 * Return: 0 on success
 *
 * Description: If the file is not an ELF File of
...
void _check_elf(unsigned char *e_ident)
{
	...
	while (no < 4)
	{
		if (e_ident[no] != 127 &&
		    ...
		    e_ident[no] != 'F')
		{
			dprintf(STDERR_FILENO, "Error: Not an ELF file]n");
			exit(98);
		}
		no++;
	}
...
/**
 * printf_magic - Prints the magic number of an ELF header.
 ...
 * @e_ident: A pointer to an array containigthe EFL class
 */
void printf_class(unsigned char *e_ident)
{
	printf("  Class:			");
 ...
 * @e_ident: A pointer to an array containong the ELF ABI version.
 */
void printf_abi(unsigned char *e_ident)
{
...
/**
 * printf_entry - Prints the entry point of an ELF headere
 ...
 *
 * Description: If the file cannot be closded - exit code 98
 */
void _close_elf(int _elf)
{
	if (close(_elf) == -1)
